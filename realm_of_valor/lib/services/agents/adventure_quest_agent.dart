...
class AdventureQuestAgent extends BaseAgent {
  ...
  void _addQuest(Event evt) {
    ...
    final objectives = objectivesRaw
        ...
        .toList();
    final q = Quest(id: id, title: title, type: type, status: QuestStatus.notStarted, objectives: objectives, rewardXp: rewardXp);
    _active[q.id] = q;
  ...
  void _progressByType(Event evt, QuestType type) {
    ...
    for (final entry in entries) {
      ...
      _active[q.id] = Quest(id: q.id, title: q.title, type: q.type, status: newStatus, objectives: updatedObjs, rewardXp: q.rewardXp);
      ...
      if (done) {
        ...
        if (q.rewardXp > 0) {
          bus.publish(Event(type: 'battle_result', data: {'win': true, 'xp': q.rewardXp}));
        }
      }
    }
  }
